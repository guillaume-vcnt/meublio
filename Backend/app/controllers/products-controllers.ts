...
const getProductByCategory = (req: Request, res: Response) => {
	Product.getProductByCategoryInfo(req.params.type, (err, data) => {
		if (err) {
			res.status(500).send({
				message:
					err.message ||
					"Some error occured while getting product info with type : ${req.params.type}.",
			});
